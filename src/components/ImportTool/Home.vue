  ...
  methods: {
      ...
      for (let index = 0; index < 15; index++) {
        ...
        if (index <= this.rowNumber) {
          ...
          if (formatedData.projects_id_number_cpa) {
            ...
            await this.updateProject(formatedData, project)
          } else {
            continue
          }
          console.log('1')
          ...
          if (this.types.includes('spendings')) {
            ...
            if (this.currentCompany.id === '809') {
              if (!formatedData.spendingLineItem_commitment_id_number) {
                ...
                formatedData.spendingLineItem_commitment_id_number = `M6ID-${Math.floor(
                  Math.random() * 1000000
                ) + 1}`
    ...
    async createCommitmentLineItem(projectID, commitmentID, item) {
      const newCommitmentLineItem = {
        ...
        startDateText: item.commitmentLineItem_start_date || '',
        startDate: new Date(item.commitmentLineItem_start_date).getTime() || '',
        vendor: item.others_vendor_code || '',
    ...
    async updateProject(project, item) {
      const proj = {
        campus: item.projects_campus || project.campus || '',
        category: item.projects_category || project.category || '',
        company_nid: this.currentCompany.id || project.company_nid || '',
        contractor: item.projects_contractor || project.contractor || '',
        description: item.projects_description || project.description || '',
        endDate: item.projects_end_date || project.endDate || '',
        endDateText: item.projects_end_date_text || project.endDateText || '',
        files: item.projects_files || project.files || '',
        manager: item.projects_manager || project.manager || '',
        totalPlanBudget: item.projects_total_plan_budget || project.totalPlanBudget || '',
        number: item.projects_id_number_cpa.toString() || project.number || '',
        phase: item.projects_phase || project.phase || '',
        phaseTargetDate: item.projects_phase_target_date || project.phaseTargetDate || '',
        phaseTargetDateText: item.projects_phase_target_date_text || project.phaseTargetDateText || '',
        projectType: item.projects_projectType || project.projectType || '',
        project_invoice_total: item.projects_project_invoice_total || project.project_invoice_total || '',
        project_total_open_po_w_tax:
            item.projects_total_open_po_w_tax || project.item || '',
        project_total_po_amount: item.projects_total_po_amount || project.project_total_po_amount || '',
        projectImage: item.projects_project_image || project.projectImage || '',
        proposedSpend: item.projects_proposed_spend || project.proposedSpend || '',
        sameAsCampusAddress: item.projects_same_as_campus_address || project.sameAsCampusAddress || '',
        setDefaultSchedule: item.projects_set_default_schedule || project.setDefaultSchedule || '',
        squareFootage: item.projects_square_footage || project.squareFootage || '',
        standard: item.projects_standard || project.standard || '',
        startDate: item.projects_startDate || project.startDate || '',
        startDateText: item.projects_start_date_text || project.startDateText || '',
        status: item.projects_status || project.status,
        teamMembers: item.projects_team_members || project.teamMembers || '',
        title: item.projects_title || item.projects_id_number_cpa.toString() || project.title,
        range: item.projects_range || project.range || '',
        priorSpend: item.projects_prior_spend || project.priorSpend || '',
        basic: {
          cPA: item.projects_id_number_cpa.toString() || project.number || '',
