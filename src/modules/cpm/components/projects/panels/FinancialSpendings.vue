<template>
  <m6-card-dialog
    ...
    :searchField="true"
    :filterColumnList="[{text: 'All', value: 'all'}, {text: 'Document/Invoice Number', value: 'number'}]"
  >
    <template v-slot:after:title>
      ...
      <v-menu v-if="search" nudge-top="100" offset-y open-on-hover>
      ...
      <template v-if="search">
        <v-tooltip left>
          <template v-slot:activator="{ on }">
            <v-icon class="close-search" v-on="on" @click="clearSearch">
              mdi-close
            </v-icon>
          </template>
      ...
      <v-icon class="cursor grey--text text--darken-2" dark @click="openModal">
        mdi-plus-circle
      </v-icon>
      <v-tooltip bottom>
        <template v-slot:activator="{ on }">
    ...
    <v-row justify="center">
      <v-chip color="transparent" disabled text-color="black">
        <strong>
    ...
    >
      ...
      <template slot="item" slot-scope="props">
        <tr @click="fetchLineItems(props)">
          ...
          <td v-if="isFullScreen">
            <!-- {{ $h.dg(props.item, "commitment.number", "") }} -->
          ...
          <td v-if="isFullScreen" class="text-center">
      ...
      <template v-if="isFullScreen" v-slot:expand="props">
        <v-row justify="center">
          <v-col cols="11">
            <v-progress-circular
    ...
    <v-dialog
      ...
      persistent
    >
      <v-card v-show="method === 'add' || method === 'put'" class="w-full">
        ...
        <v-card-text class="card-scroll vertical-scroll">
          <v-form ref="form" v-model="valid">
            <v-container class="pa-0" fluid>
              <v-row align="center" justify="center">
                ...
                <v-col cols="7">
                  ...
                  >
                    ...
                    <v-date-picker v-model="openDate" color="green" scrollable>
                      <v-spacer />
                      <v-btn
                        color="primary"
                        text
              ...
              <v-row v-if="glaccount" align="center" justify="center">
                <v-col cols="3">
                  <div class="font-weight-black subheading">
                    <v-row>
                      <v-col class="align-center d-flex text-nowrap">
              ...
              <v-row align="center" justify="center">
                ...
                <v-col cols="7">
                  ...
                  >
                    ...
                    <v-date-picker v-model="paidDate" color="green" scrollable>
                      <v-spacer />
                      <v-btn
                        color="primary"
                        text
          ...
          <v-container class="pa-0" fluid grid-list-md>
            ...
            <v-row v-show="filesToAdd.length" align="center" justify="center">
              <v-col cols="3">
                <div class="font-weight-black subheading">
                  <v-row>
                    <v-col class="align-center d-flex text-nowrap">
              ...
              <v-col cols="7">
                ...
                >
                  ...
                  <template slot="append" slot-scope="{ item }">
                    <template v-if="item.file">
                      <v-btn icon small @click.stop="deleteFileNotSaved(item)">
                        <v-icon color="red" small> mdi-delete </v-icon>
                      </v-btn>
                    </template>
                  </template>
    ...
    >
      <v-card>
        ...
        >
          <v-form ref="formLineItem" v-model="validLineItem">
            <v-container class="pa-0" fluid grid-list-md>
              <v-row>
                <v-col md="6" sm="12">
                  <v-text-field
        ...
        <v-card-actions>
          ...
          >
            <template v-if="methodLineItem === 'add'"> Add </template>
            <template v-if="methodLineItem === 'put'"> Update </template>
          </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    >
      <v-card class="ml-2">
        ...
        <v-card-actions class="px-5">
          ...
          <v-btn text @click="showLineItemsModal = false"> close </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <!-- show line items modal -->
  ...
  methods: {
    ...
    getParseSpending() {
      ...
      return {
        ...
        commitment: this.$h.dg(this.dialogProperties, "commitment.id")
          ...
          : {},
        poNumber: this.dialogProperties.commitment.number || "",
    ...
    async openUpdateSpending(spending = {}) {
      ...
      if (spending.commitment) {
        const commitment = this.commitments.find(
          (commitment) => commitment.id === spending.commitment.id
        );
        if (commitment) this.dialogProperties.commitment = commitment;
      }
    ...
    filterItems: function(arr, query) {
      ...
      return arr.filter(function(item) {
        ...
        return (columnList.number || columnList.vendors || columnList.po_number || columnList.dateOpenedText || 
                columnList.postingDateText || columnList.amount);
      })
    },
  },
};
