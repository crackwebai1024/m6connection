<template>
  <div>
    ...
    <v-card v-if="showForm" class="mt-2">
      <v-card-text class="vertical-scroll">
        <v-form ref="form">
          <h6 v-if="fundingStatus" class="indigo--text">
            Editing:
          </h6>
          <v-text-field
            v-model="status"
          ...
          />
          <input v-model="fundingStatus" type="hidden" />
          <v-btn color="blue" outlined @click="cancel">
            Cancel
          </v-btn>
          <v-btn color="blue" dark type="button" @click="saveFundingStatus">
