<template>
  <v-container class="max-w-container mx-auto pa-0">
    <m6-list
    ...
    >
      ...
      <template v-slot:after:search>
        <v-col id="step4" class="align-center mx-4 shrink">
          <list-filtering
        ...
        <v-col id="step5" class="shrink" cols="2">
          <v-select
            v-model="view"
            item-text="label"
            item-value="val"
              ...
              >
                ...
                >
                  <v-row class="justify-center pb-0 pt-3">
                    <v-badge offset-x="21" offset-y="25">
                ...
                <div class="d-flex ma-0 pa-0">
                  <div
                    class="grey--text py-1 text--darken-3 text-caption text-center w-half"
                  >
                    {{ $t("cpm.budgetStatus") }}
                  </div>
                  <div
          ...
          >
            <template slot="item" slot-scope="{ item }">
              <tr>
                ...
                <td>
                  <a href="#" @click.prevent="goToProject(item, false)">
                ...
                <td>
                  <v-menu nudge-top="70" offset-y open-on-hover>
                    <template v-slot:activator="{ on }">
                      <div class="maxWidth9rem text-truncate" v-on="on">
                        {{ item.title }}
                      </div>
                    </template>
                ...
                <template v-if="isPlanned">
                  <td v-for="(fy, index) in fiscalYears" :key="index">
                    {{
    ...
    <cpm-create :show="showCreateModal" @close="showCreateModal = false" />
    ...
    >
      ...
      <ul class="mb-4 mt-2">
        <li v-for="rfp in associatedRfps" :key="rfp.id">
          <span>{{ rfp.name }}</span>
        </li>
      </ul>

      <span class="blue--text pointer" @click="goToProposal">Click here</span>
