        ...
        <v-col cols="10">
          ...
          >
            ...
            <template v-slot:item.rapid_reaction="{ item }" >
              <span class="emoji-height" v-html="emojis.find( e => e.level == item.rapid_reaction ).emoji" />
            </template>
            <template v-slot:item.actions="{ item }" >
              <router-link :to="{ name: 'record.show', params: { id: item.record_id } } " >
                <v-btn icon fab x-small color="green darken-1">
                  <v-icon>mdi-pencil</v-icon>
                </v-btn>
              </router-link>
              <v-btn icon fab x-small color="red darken-1" @click="showDeleteRapidDialog = true">
